...
protocol FAScrollViewOutput: class {
...
class FAScrollView: UIScrollView{
    ...
    weak var output: FAScrollViewOutput?
    ...
    func resizeWithNoSpace(animated: Bool = false) {
        setZoomScale(zoomScaleWithNoWhiteSpaces(), animated: animated)
        updateLayout()
    }
    
    func resizeWithMinimum(animated: Bool = false) {
        setZoomScale(minimumZoomScale, animated: true)
        updateLayout()
    }
    
    func resetCurrentImageSize() {
        zoomScale = 1.0
        minimumZoomScale = 1.0
        imageView.frame.size = sizeForImageToDisplay()
        imageView.center = center
...
extension FAScrollView:UIScrollViewDelegate{
    ...
    func scrollViewDidEndZooming(_ scrollView: UIScrollView, with view: UIView?, atScale scale: CGFloat) {
        print("ended scrollViewDidEndZooming")
        output?.didEndImageDeceleration()
    }
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        print("ended scrollViewDidEndDecelerating")
        output?.didEndImageDeceleration()
    }
